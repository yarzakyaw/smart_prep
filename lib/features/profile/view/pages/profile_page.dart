                    ...
                    ElevatedButton(
                      onPressed: () {
                        ...
                        /* Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) =>
                                const SignUpPage(isAnonymousConversion: true),
                          ),
                        ); */
                        Get.toNamed(
                          '/signup',
...
class ProfilePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Profile', style: TextStyle(fontFamily: tFont)),
      ),
      body: authState.when(
        data: (user) {
          if (user == null) {
              ...
              return Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'UID: ${user.uid}',
                      style: const TextStyle(fontSize: 16, fontFamily: tFont),
                    ),
                    if (!user.isAnonymous) ...[
                      ...
                      const SizedBox(height: 8),
                      Text(
                        'Teacher Status: ${profile?['isTeacher'] == true ? 'Teacher' : 'Student'}',
                        style: const TextStyle(fontSize: 16, fontFamily: tFont),
                      ),
                    ],
                    const SizedBox(height: 16),
                    ...
                    if (!user.isAnonymous)
                      ElevatedButton(
                        onPressed: () {
                          // Placeholder for teacher privilege request (to be implemented in Step 3.5)
                          SnackbarGetxController.successSnackBar(
                            title: 'Attention',
                            message: 'Teacher privilege request coming soon!',
                          );
                          /* ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Teacher privilege request coming soon!',
                                style: TextStyle(fontFamily: tFont),
                              ),
                            ),
                          ); */
